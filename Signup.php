<?php
  ...
  if (isset($_POST['reg'])) {
    ...
    $customar_id =mt_rand(1,1000);

    $qry = "INSERT INTO customars (username, email, password, city, address , id_num) 
    VALUES ('$username', '$email','$password','$city','$address' ,'$customar_id')";
    $run = mysqli_query($con , $qry);

    if ($run==true) {
      echo "<script> alert('Kepp this ID ( $customar_id ) Everywhere to Buy our Product')  </script>";
    }
    else{
